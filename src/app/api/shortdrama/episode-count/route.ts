...
export async function GET(request: NextRequest) {
  try {
    ...
    // 使用备用API搜索短剧获取集数
    const searchUrl = `${alternativeApiUrl}/api/v1/drama/d?dramaName=${encodeURIComponent(
      name
    )}`;
    ...
    if (!searchResponse.ok) {
      // 精确搜索失败，尝试模糊搜索
      const fuzzySearchUrl = `${alternativeApiUrl}/api/v1/drama/dl?dramaName=${encodeURIComponent(
        name
      )}`;
